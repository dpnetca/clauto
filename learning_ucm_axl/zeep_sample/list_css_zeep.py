...
list_css_data = {"searchCriteria": {"name": "%"}, "returnedTags": {}}

css_list = service.listCss(**list_css_data)

for css in css_list["return"]["css"]:
    css_detail = service.getCss(uuid=css["uuid"])
    print(css_detail["return"]["css"]["name"])
    for pt in css_detail["return"]["css"]["members"]["member"]:
        print(f" - {pt['routePartitionName']['_value_1']}")
    print("\n")
